...
${FFMPEG} -threads $THREADS -i "$FILE" -vf "vidstabtransform=input=$STAB:zoom=1:smoothing=30,unsharp=5:5:0.8:3:3:0.4" -vcodec libx264 -preset slow -tune film -crf 18 -acodec copy "${PARAMS%.*}_stabilized.mkv"
rm -f $STAB
