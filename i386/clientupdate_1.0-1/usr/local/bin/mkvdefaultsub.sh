#!/bin/bash
TRACKNUMBER=$( LANG=C mkvinfo -s $2 | head -n 30 | grep subtitle | grep "language: $1" | head -n 1 | cut -d: -f4 | cut -d, -f1 | xargs )
if [[ -z $TRACKNUMBER ]]; then
	...
	mkvpropedit "$2" --edit track:s$TRACKNUMBER --set flag-default=1 1>/dev/null 2>&1
	echo "done."
